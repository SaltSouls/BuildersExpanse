...
public class HammerItem extends DiggerItem {
    ...
    public HammerItem(Tier tier, float attackDamage, float attackSpeed, Properties properties, TagKey<Block> blocks, int radius, int depth) {
    ...
    public boolean mineBlock(@NotNull ItemStack item, @NotNull Level level, @NotNull BlockState state, @NotNull BlockPos pos, @NotNull LivingEntity entity) {
        if (level.isClientSide && state.getDestroySpeed(level, pos) != 0.0F && !state.is(this.blocks)) {
            return true;
        }

        float hardness = state.getDestroySpeed(level, pos);
        this.breakExtras(entity, getRadius(this.radius), getDepth(this.depth), hardness, this.blocks);
    ...
    public void harvestBlocks(@NotNull Player player, ServerLevel level, BlockState state, BlockPos pos) {
        ...
        if (!player.isCreative() && isEffective(player, pos, this.blocks)) {
            ...
            drops.forEach(items -> Block.popResourceFromFace(level, pos, getTargetedBlock(player).getDirection(), items));
        }
    }
     ...
     */
    public static boolean isHammer(@NotNull ItemStack item) {
        return item.getItem() instanceof HammerItem;
    }
     ...
     */
    public static boolean canBulkMine(@NotNull BlockState state, TagKey<Block> tag) {
        return state.is(tag);
    }
     ...
     */
    static int getAdjacent(int coord, int radius) {
        return coord + radius;
    }
     ...
     */
    static int getOpposite(int coord, int radius) {
        return coord - (radius * 2);
    }

    static int getRadius(int radius) {
        return radius - 2;
    }

    static int getDepth(int depth) {
        return depth - 1;
    }
